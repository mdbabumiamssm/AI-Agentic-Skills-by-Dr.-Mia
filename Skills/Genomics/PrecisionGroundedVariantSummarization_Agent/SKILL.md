...
9. **Precision Grounding workflow** - Use Precision Grounding as the named workflow for trustworthy genetic variant summarization: retrieve evidence from databases and literature, link each generated claim to provenance, reconcile conflicts across ClinVar and literature while keeping other evidence streams separate when consulted, run hallucination checks against retrieved evidence, and produce clinician-reviewable variant summaries.
10. **Precision Grounding evaluation safeguards** - Preserve and explicitly report conflicting classifications; check for stale records using available update, version, or access dates; audit citation completeness for clinically meaningful claims; test generated summaries for unsupported or hallucinated statements; and require domain-expert review of generated variant summaries and unresolved conflicts.
11. **Version-aware evidence reconciliation and abstention** - Normalize variant identity before joining records; join evidence only across explicit genome build, transcript, allele, condition, and database version or access-date context; preserve conflicting classifications instead of collapsing them; attach claim-level provenance and traceable citations; flag evidence freshness limitations; and abstain from asserting a classification or relationship when support is insufficient, ambiguous, or not traceable.
12. **Evidence-database augmentation pattern** - Rank sources by fit to the variant question and requested context; preserve ClinVar classifications and submitter or expert-review context, gnomAD population-frequency evidence, and literature provenance as separate evidence streams; surface conflicts instead of smoothing them over; assign transparent confidence labels tied to the retrieved evidence; and use hallucination-resistant output templates with required provenance fields and explicit "not found in consulted sources" labels.
13. **Precision Grounding evidence tables** - Treat Precision Grounding as the evidence-database augmentation pattern for trustworthy variant summaries: build provenance tables for each claim; keep ClinVar classifications, gnomAD population-frequency evidence, and literature findings as source-specific records; reconcile conflicts without collapsing disagreement; run hallucination checks against retrieved evidence; and report structured uncertainty for unsupported, ambiguous, stale, or conflicting evidence.
14. **Precision Grounding summarization controls** - Retrieve variant evidence from evidence-based databases before drafting; report source provenance, record identifiers, conflicts, and source version or access-date context with each claim; separate retrieved evidence from model interpretation; and block or revise any statement that cannot be traced to consulted evidence.
15. **Evidence-backed Precision Grounding validation** - Use Precision Grounding as a variant-summarization pattern that validates each draft summary against trusted clinical genetics sources, preserves database provenance, detects conflicting records or assertions, and revises or withholds claims that fail hallucination checks against the retrieved evidence.
