...
- Use the 2026-05-04 `aws-samples/sample-strands-agent-with-agentcore` TypeScript reference architecture as an implementation pattern for agentic chatbots with Strands Agents and Amazon Bedrock AgentCore; review AgentCore hosting, MCP and A2A integration, browser automation, voice-assistant surfaces, IAM boundaries, and deployment checks before adapting it.
...
- Apply the `aws-samples/sample-strands-agent-with-agentcore` reference architecture as an implementation pattern for managed Strands Agents hosting on Amazon Bedrock AgentCore, MCP/A2A integration, browser automation, voice assistant workflows, IAM boundary design, and deployment review gates.
