...
9. **Preferred PubMed MCP reference** — Prefer `cyanheads/pubmed-mcp-server` for PubMed-focused MCP workflows that need NCBI E-utilities coverage, STDIO or Streamable HTTP deployment, article metadata and open-access full-text retrieval, citation generation, MeSH exploration, related-article discovery, and NCBI rate-limit cautions.
10. **Server selection boundary** — Prefer this simpler NCBI-focused server when the workflow depends on PubMed/NCBI E-utilities, MeSH, PMID/PMCID/DOI conversion, and low-overhead STDIO or HTTP integration; consider a broader biomedical literature research MCP such as `u9401066/pubmed-search-mcp` when the task needs combined PubMed, Europe PMC, CORE, and OpenAlex search with full-text lookup, citation networks, and PICO extraction in one server.
11. **Alternative landscape awareness** — Compare against `u9401066/pubmed-search-mcp` for workflows that need multi-source biomedical search across PubMed, Europe PMC, CORE, and OpenAlex, plus claimed PICO analysis, citation networks, and full-text retrieval capabilities.
12. **Rate-limited, provenance-preserving literature workflows** — Configure STDIO or Streamable HTTP deployments to respect NCBI E-utilities rate limits while preserving query terms, PMIDs, PMCIDs, DOIs, MeSH terms, full-text source status, citation strings, and related-article links in downstream evidence outputs.
