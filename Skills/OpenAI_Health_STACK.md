...
| Health Data Connector | `platform/biokernel/health_data_connector.py` *(planned)* | Streams FHIR and wearable data into BioKernel topics so all agents can subscribe. |
...
3. **Enforce Schema** – Each module validates outputs locally before the response hits BioKernel, mirroring OpenAI's schema contracts.
...
* Add a new router profile in BioKernel: `openai-health` to route to GPT-4.1 or GPT-4o depending on latency vs accuracy requirements.
