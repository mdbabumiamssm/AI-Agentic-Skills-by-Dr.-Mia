...
15. **Evaluate AKM as a candidate agent-skill package manager.** Include AKM (Agent Kit Manager) as a concrete GitHub-hosted TypeScript package-manager example for AI agent skills, commands, tools, and knowledge. Treat it as a package-management workflow rather than a marketplace-only catalog: inspect its manifest structure, cross-agent layout compatibility, version pinning, trust checks, lockfile or rollback support, source provenance, safe install/remove behavior, dependency hygiene, install/sync/update flows, install isolation, namespace collision and conflict handling, and how it compares with existing skill manager options such as ccpi/craftdesk-style tools before adoption. Do not assume feature parity; verify AKM behavior from the repository or local docs before relying on it.

16. **Compare Craftdesk package-manager patterns.** Include Craftdesk as a TypeScript package-manager example for Claude Code skills, agents, and related AI resources. In the comparison matrix, review dependency metadata, reproducible installation controls, version and lockfile expectations, registry and source trust review, uninstall behavior, rollback expectations, and conflict detection across agent runtimes when multiple skill sources target the same skill, agent, command, file, or registration.
