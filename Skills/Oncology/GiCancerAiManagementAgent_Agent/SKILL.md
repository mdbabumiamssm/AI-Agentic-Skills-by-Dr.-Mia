...
9. **ASCO GI cancer management coverage** - Use the 2026 ASCO Educational Book review on AI for GI cancer management as high-level evidence context when scoping endoscopy, pathology, radiology/imaging, molecular profiling, treatment planning, trial selection, surveillance, clinician governance, and governance boundaries; keep outputs framed as clinician-governed decision support and explicitly surface evidence, validation, bias, implementation, generalizability limitations, and limits on autonomous recommendations.
