...
4.  **Dermatology and Dermoscopy Evaluation Profile**: For suspected basal cell carcinoma and predefined common mimicker sets, evaluate clinical-only, dermoscopic-only, and paired clinical-plus-dermoscopic inputs when available; stratify image quality using focus, lighting, scale, lesion completeness, artifacts, and occlusion, and reject or abstain when quality is insufficient; record available lesion metadata and lesion-level visual evidence; frame outputs as triage support with a ranked lesion-level differential diagnosis, per-diagnosis scores, and calibrated confidence rather than a standalone definitive diagnosis; measure lesion-level sensitivity and specificity on clinician-confirmed evaluation datasets; compare models on the same lesion cases, modality conditions, and quality-control criteria; conduct clinician-reviewed lesion-level error analysis, including confusion among mimickers; and require dermatologist review of clinically dangerous false negatives and all outputs before clinical use.
