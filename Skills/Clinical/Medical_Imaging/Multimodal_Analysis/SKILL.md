...
4.  **Dermatology and Dermoscopy Evaluation Profile**: For suspected basal cell carcinoma and predefined common mimicker sets, evaluate paired clinical and dermoscopic images when available; reject or abstain on images with insufficient focus, lighting, scale, lesion completeness, or excessive artifacts or occlusion; record available lesion metadata and lesion-level visual evidence; frame outputs as triage support with a ranked differential diagnosis, per-diagnosis scores, and calibrated confidence rather than a standalone definitive diagnosis; assess calibration and clinician-confirmed evaluation metrics such as sensitivity and specificity on evaluation datasets; conduct clinician-reviewed lesion-level error analysis, including confusion among mimickers; and require dermatologist review before clinical use.
