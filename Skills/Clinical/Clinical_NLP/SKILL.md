...
9.  **Longitudinal Outcome-Event Identification and Timing**: Extract outcome-event status with onset and resolution dates, represent uncertain timing as intervals, separate note time from event time, deduplicate the same event across longitudinal notes, and retain clinician-auditable evidence spans for every structured event.
