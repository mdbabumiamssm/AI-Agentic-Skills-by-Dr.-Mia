...
7.  **Rare-Neoplasm Registry RWD Extraction**: For low-prevalence cancer cohorts such as bone sarcoma, define the registry schema and field-level data dictionary before extracting pathology type, tumor size, localization, grade, primary resection, treatments, and outcomes; extract and normalize temporal events; distinguish absent documentation, unknown values, and explicit negative findings; flag ambiguous values; validate a sampled set through clinical adjudication; report field-level and aggregate precision and recall without assuming preset benchmarks; and export real-world records with source-note identifiers, evidence spans, temporal anchors, missingness states, and adjudication status preserved as provenance.
