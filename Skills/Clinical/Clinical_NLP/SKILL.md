...
7.  **Rare-Neoplasm Oncology RWD Extraction**: For low-prevalence cancer cohorts such as bone sarcoma, extract registry-style fields such as pathology type, tumor size, localization, grade, and primary resection; normalize temporal anchors for outcomes and treatments; capture provenance to source notes and evidence spans; separate missing from negative findings; add uncertainty or needs-review flags for ambiguous values; and require clinician validation loops for LLM-assisted real-world data abstraction.
